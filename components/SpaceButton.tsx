...
// Pre-calculate random values to avoid hydration mismatches
...
const generateExplosions = (count: number) => {
    return Array.from({ length: count }).map((_, i) => ({
        ...
        delay: Math.random() * 2,
        color: Math.random() > 0.5 ? '#ef4444' : '#fb923c' // Red or Orange
    }));
};
    ...
    return (
        ...
        >
            ...
            <div className="absolute inset-0 -z-10 bg-black w-full h-full overflow-hidden">
                ...
                {/* 3. Tiny Fighter Ship Silhouette (Leading the charge) */}
                <motion.svg
                    viewBox="0 0 24 24"
                    ...
                    variants={{
                        hover: {
                            x: ['-50px', '400px'],
                            y: [0, -10, 10, 0], // Slight banking maneuver
                            rotate: [90, 85, 95, 90], // Slight wobble
                            transition: {
                                duration: 1.2,
                                repeat: Infinity,
                                ease: "easeInOut",
